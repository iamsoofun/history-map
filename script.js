...
const REGION_MAP = {
    ...
    "Albania": "Europe", "Montenegro": "Europe", "North Macedonia": "Europe",
...
const map = L.map("map", { zoomControl: false }).fitBounds([[-58, -170], [78, 170]]);

L.control.zoom({ position: "bottomright" }).addTo(map);

// Keyless OpenStreetMap tiles, darkened via a CSS filter on the map pane
// (see .leaflet-tile in style.css). Chosen over CARTO's dark_all basemap,
// which now requires a registered API key. OSM standard tiles need no key.
...
const countryCoverageTable = document.getElementById("countryCoverageTable");
...
const ANALYSIS_CANVAS_IDS = [
    "chartIncidentsByDecade", "chartIncidentsByCountry", "chartFatalitiesByCountry", "chartIncidentsByRegion",
    "chartFatalitiesByDecade", "chartInjuriesByDecade", "chartIncidentsByCategory", "chartFatalitiesByCategory",
...
// Historical Research Range panel: collapsed by default (see index.html)
// so it doesn't permanently cover part of the map — especially costly on
// mobile, where the panel goes full-width across the bottom of an already
// small viewport. Same collapse pattern as the legend above.
const dateRangeResearch = document.getElementById("dateRangeResearch");
const dateRangeToggle = document.getElementById("dateRangeToggle");
if (dateRangeResearch && dateRangeToggle) {
    dateRangeToggle.addEventListener("click", () => {
        const nowCollapsed = dateRangeResearch.classList.toggle("collapsed");
        dateRangeToggle.setAttribute("aria-expanded", String(!nowCollapsed));
    });
}
...
fetch("history.json")
    ...
    .then(data => {
        ...
        renderCountryCoverageTable();
        populateForecastCountries();
        populateDashboardShell(); // must run after events is actually populated — see note at its definition
    })
    .catch(error => {
        console.error(error);
...
// Ported directly from the offline source_quality.py tool, so the live
// site's "sourced/verified" numbers match what that tool reports rather
// than drifting into a second, inconsistent definition. See that file
// for the full rationale (raw source count is a poor reliability proxy;
// a dedicated Wikipedia article or an official/primary source is a
// stronger single citation than several weak ones).
const PRIMARY_DOMAIN_PATTERNS = [
    /\.gov(\/|$)/, /\.gov\./, /fbi\.gov/, /un\.org/, /police\./,
    /nps\.gov/, /justice\.gov/, /parliament\./, /rcmp/,
];
...
const PRIMARY_ORG_NAME_PATTERNS = [
    /federal bureau of investigation/, /\bfbi\b/,
    /department of (public )?safety/, /police (department|service)/,
    /sheriff'?s office/, /\brcmp\b/, /royal canadian mounted police/,
    /government of/, /\bbureau of\b/, /attorney general/,
    /coroner/, /\bmedical examiner\b/, /district attorney/,
    /\bcourt\b/, /parliamentary records/, /mass casualty commission/,
    /serious incident response team/, /provincial police/,
    /\bcity of\b.*council/, /national park service/,
];
const MAJOR_NEWS_ORG_NAME_PATTERNS = [
    /associated press/, /\breuters\b/, /\bbbc\b/, /canadian press/,
    /\bcnn\b/, /\bnpr\b/, /\bap news\b/,
];

function classifySource(urlOrText) {
    const s = String(urlOrText || "").toLowerCase();
    if (s.includes("wikipedia.org")) {
        return /list_of|list%20of/.test(s) ? "wiki_list" : "wiki_article";
    }
    if (PRIMARY_DOMAIN_PATTERNS.some(p => p.test(s))) return "primary_official";
    if (PRIMARY_ORG_NAME_PATTERNS.some(p => p.test(s))) return "primary_official";
    if (MAJOR_NEWS_DOMAINS.some(d => s.includes(d))) return "major_news";
    if (MAJOR_NEWS_ORG_NAME_PATTERNS.some(p => p.test(s))) return "major_news";
    return "other";
}

function sourceQualityTier(event) {
    const sources = Array.isArray(event.sources) ? event.sources : [];
    if (sources.length === 0) return "MISSING";
    if (sources.length >= 2) return "STRONG";
    const kind = classifySource(sources[0]);
    if (kind === "wiki_list") return "WEAK";
    if (kind === "wiki_article" || kind === "primary_official" || kind === "major_news") return "STRONG";
    return "UNCLEAR";
}
...
function sourceCoverageCategory(event) {
    ...
    const tier = sourceQualityTier(event);
    return tier === "STRONG" ? "cited" : "partial";
}

function buildCountryCoverageIndex() {
    const byCountry = {};
    events.forEach(e => {
        const c = e.country || "Unknown";
        (byCountry[c] = byCountry[c] || []).push(e);
    });

    const rows = Object.entries(byCountry).map(([country, recs]) => {
        const years = recs.map(r => r.year).filter(Number.isFinite);
        const withCoords = recs.filter(r => r.lat && r.lng).length;
        const strong = recs.filter(r => sourceQualityTier(r) === "STRONG").length;
        const multiSource = recs.filter(r => Array.isArray(r.sources) && r.sources.length >= 2).length;
        return {
            country,
            incidents: recs.length,
            earliest: years.length ? Math.min(...years) : null,
            latest: years.length ? Math.max(...years) : null,
            pctCoords: recs.length ? Math.round((withCoords / recs.length) * 100) : 0,
            pctMultiSource: recs.length ? Math.round((multiSource / recs.length) * 100) : 0,
            pctVerified: recs.length ? Math.round((strong / recs.length) * 100) : 0,
        };
    });
...
function renderCountryCoverageTable() {
    if (!countryCoverageTable) return;
    const rows = buildCountryCoverageIndex();
    if (!rows.length) {
        countryCoverageTable.innerHTML = `<p class="dq-empty">No records loaded.</p>`;
        return;
    }
    ...
    const body = rows.map(r => `
        <tr>
            ...
            <td>${r.incidents.toLocaleString()}</td>
            <td>${r.earliest ?? "—"}</td>
            <td>${r.latest ?? "—"}</td>
            <td>${r.pctCoords}%</td>
            <td>${r.pctMultiSource}%</td>
            <td>${r.pctVerified}%</td>
        </tr>
    `).join("");

    countryCoverageTable.innerHTML = `
        <table class="cct-table">
            <thead>
                <tr>
                    <th>Country</th><th>Incidents</th><th>Earliest</th><th>Latest</th>
                    <th>Coords</th><th>2+ sources</th><th>Verified</th>
                </tr>
            </thead>
            <tbody>${body}</tbody>
        </table>
...
// Record coverage over time — the measurement environment, shown BEFORE
// any ranking derived from it. Deliberately whole-dataset (not filtered):
// this describes what NHIRA has collected, not what the current research
// filters select. The accompanying note states the largest decade-to-decade
// jump in plain terms, because that jump is the clearest single indication
// that record counts track collection history and not only incidence.
// Today in History — incidents matching the current month-and-day across
// all years. A pure date filter over the record: no statistical claim, no
// forecast, no ranking. If nothing matches today's exact date, it widens to
// the surrounding +/-3 days so the panel is informative rather than empty,
// and says so. Clicking an entry opens that incident's full record.
// Explore the Record — rankings over a FROZEN unit universe, per the
// Discovery Boundary Rule. Units are fixed and pre-set (decades, countries,
// US states, top categories); the engine ranks WITHIN them and never
// searches for its own regions or windows. Every ranking discloses the size
// of its comparison set ("of N decades"), and extrema are stated as such.
// No per-capita rates (no population denominators exist).
const EXPLORE_STATE = { tab: "time" };

function exploreRankRows(pairs, unitNoun, total, unit) {
    // pairs: array of [label, value] already sorted desc; render top 8
    const top = pairs.slice(0, 8);
    const max = top.length ? top[0][1] : 0;
    return top.map(([label, value], i) => {
        const pct = max > 0 ? Math.round((value / max) * 100) : 0;
        return `<li class="explore-row">
            <span class="explore-rank">${i + 1}</span>
            <span class="explore-label">${escapeHtml(label)}</span>
            <span class="explore-bar-wrap"><span class="explore-bar" style="width:${pct}%"></span></span>
            <span class="explore-value">${value.toLocaleString()}${unit ? " " + escapeHtml(unit) : ""}</span>
        </li>`;
    }).join("");
}

function exploreBlock(title, comparisonText, rowsHtml) {
    return `<div class="explore-block">
        <p class="explore-block-title">${escapeHtml(title)}</p>
        <p class="explore-compare">${escapeHtml(comparisonText)}</p>
        <ol class="explore-list">${rowsHtml}</ol>
    </div>`;
}

function renderExploreRecord() {
    const body = document.getElementById("exploreBody");
    if (!body) return;
    const tab = EXPLORE_STATE.tab;
    const withYear = events.filter(e => Number.isFinite(e.year));
    const fat = e => (Number(e.fatalities) || 0);

    let html = "";

    if (tab === "time") {
        // fixed units: decades present in the record
        const incByDec = countBy(withYear, e => decadeOf(e.year));
        const decLabels = sortedDecadeLabels(incByDec);
        const nDec = decLabels.length;
        const incPairs = decLabels.map(d => [d + "s", incByDec[d]]).sort((a, b) => b[1] - a[1]);
        html += exploreBlock(
            "Most incidents by decade",
            `Compared across ${nDec} decades represented in the record.`,
            exploreRankRows(incPairs, "decade", nDec, "incidents"));

        const fatByDec = {};
        withYear.forEach(e => { const d = decadeOf(e.year); fatByDec[d] = (fatByDec[d] || 0) + fat(e); });
        const fatPairs = Object.entries(fatByDec).map(([d, v]) => [d + "s", v]).sort((a, b) => b[1] - a[1]);
        html += exploreBlock(
            "Most fatalities by decade",
            `Compared across ${nDec} decades represented in the record.`,
            exploreRankRows(fatPairs, "decade", nDec, "fatalities"));

    } else if (tab === "geography") {
        // fixed units: countries, then US states
        const incByCountry = countBy(events.filter(e => e.country), e => e.country);
        const nCountry = Object.keys(incByCountry).length;
        const cPairs = Object.entries(incByCountry).sort((a, b) => b[1] - a[1]);
        html += exploreBlock(
            "Most incidents by country",
            `Compared across ${nCountry} countries represented in the record.`,
            exploreRankRows(cPairs, "country", nCountry, "incidents"));

        const usStates = events.filter(e => e.country === "United States" && e.state
            && e.state !== "United States" && !String(e.state).includes(" and "));
        const incByState = countBy(usStates, e => e.state);
        const nState = Object.keys(incByState).length;
        const sPairs = Object.entries(incByState).sort((a, b) => b[1] - a[1]);
        html += exploreBlock(
            "Most incidents by U.S. state",
            `Compared across ${nState} U.S. states/territories present in the record.`,
            exploreRankRows(sPairs, "state", nState, "incidents"));

    } else if (tab === "category") {
        // fixed units: recorded category values (excluding the empty "None")
        const withCat = events.filter(e => e.category && e.category !== "None");
        const incByCat = countBy(withCat, e => e.category);
        const nCat = Object.keys(incByCat).length;
        const catPairs = Object.entries(incByCat).sort((a, b) => b[1] - a[1]);
        html += exploreBlock(
            "Most common incident categories",
            `Compared across ${nCat} recorded category labels.`,
            exploreRankRows(catPairs, "category", nCat, "incidents"));

        const fatByCat = {};
        withCat.forEach(e => { fatByCat[e.category] = (fatByCat[e.category] || 0) + fat(e); });
        const fcPairs = Object.entries(fatByCat).sort((a, b) => b[1] - a[1]);
        html += exploreBlock(
            "Most fatalities by category",
            `Compared across ${nCat} recorded category labels.`,
            exploreRankRows(fcPairs, "category", nCat, "fatalities"));
    }

    body.innerHTML = html;
}

(function wireExploreRecord() {
    const tabs = document.querySelectorAll(".explore-tab");
    if (!tabs.length) return;
    tabs.forEach(btn => btn.addEventListener("click", () => {
        tabs.forEach(b => { b.classList.remove("active"); b.setAttribute("aria-selected", "false"); });
        btn.classList.add("active"); btn.setAttribute("aria-selected", "true");
        EXPLORE_STATE.tab = btn.getAttribute("data-explore") || "time";
        renderExploreRecord();
    }));
})();

function renderTodayInHistory() {
    const listEl = document.getElementById("todayInHistoryList");
    const dateEl = document.getElementById("todayInHistoryDate");
    const noteEl = document.getElementById("todayInHistoryNote");
    ...
    dateEl.textContent = `Recorded incidents on ${monthNames[now.getMonth()]} ${dd}.`;

    const dated = events.filter(e => /^\d{4}-\d{2}-\d{2}$/.test(String(e.date || "")));
    const onDay = e => {
        const p = String(e.date).split("-");
        return Number(p[1]) === mm && Number(p[2]) === dd;
    };
    let matches = dated.filter(onDay);
    let widened = false;

    if (!matches.length) {
        // widen to +/-3 calendar days (ignoring year) so the panel is useful
        widened = true;
        const target = new Date(2000, mm - 1, dd);
        const within = e => {
            const p = String(e.date).split("-");
            const d = new Date(2000, Number(p[1]) - 1, Number(p[2]));
            let diff = Math.abs(d - target) / 86400000;
            diff = Math.min(diff, 366 - diff); // wrap around year boundary
            return diff <= 3;
        };
        matches = dated.filter(within);
    }
    ...
    if (!matches.length) {
        ...
        if (noteEl) noteEl.textContent =
            "No incidents in the record fall on or near this date. This reflects what NHIRA has collected, not that no events ever occurred on this day.";
        return;
    }

    listEl.innerHTML = matches.map(e => {
        const place = [e.city, e.country].filter(Boolean).map(escapeHtml).join(", ");
        const cas = (Number(e.fatalities) || 0);
        const casTxt = cas > 0 ? ` \u00b7 ${cas.toLocaleString()} killed` : "";
        return `<li class="tih-item" data-incident-id="${escapeHtml(e.id)}" role="button" tabindex="0">
            <span class="tih-year">${escapeHtml(e.year)}</span>
            <span class="tih-body"><span class="tih-title">${escapeHtml(e.title)}</span>
            <span class="tih-meta">${place}${casTxt}</span></span></li>`;
    }).join("");

    if (noteEl) {
        noteEl.textContent = widened
            ? `No incident falls exactly on ${monthNames[now.getMonth()]} ${dd}; showing the ${matches.length} nearest within three days.`
            : `${matches.length} incident${matches.length === 1 ? "" : "s"} recorded on this calendar day across all years.`;
    }
}

// delegate clicks/keys on Today-in-History items to the incident opener
(function wireTodayInHistory() {
    const list = document.getElementById("todayInHistoryList");
    if (!list) return;
    const open = el => {
        const id = el && el.getAttribute("data-incident-id");
        if (!id) return;
        const ev = events.find(x => String(x.id) === String(id));
        if (ev && typeof openPanel === "function") openPanel(ev);
    };
    list.addEventListener("click", e => {
        const item = e.target.closest(".tih-item"); if (item) open(item);
    });
    list.addEventListener("keydown", e => {
        if (e.key !== "Enter" && e.key !== " ") return;
        const item = e.target.closest(".tih-item"); if (item) { e.preventDefault(); open(item); }
    });
})();
...
function renderRecordCoverage() {
    ...
    const byDecade = countBy(events.filter(e => Number.isFinite(e.year)), e => decadeOf(e.year));
    const labels = sortedDecadeLabels(byDecade);
    if (!labels.length) return;

    drawChart("chartRecordCoverage", {
        type: "bar",
        data: {
            labels: labels.map(d => d + "s"),
            datasets: [{ data: labels.map(d => byDecade[d]), backgroundColor: "#6B7A8F" }]
        },
        options: CHART_BASE_OPTIONS
    });

    const note = document.getElementById("recordCoverageNote");
    if (!note) return;

    let jumpFrom = null, jumpTo = null, jumpSize = 0;
    for (let i = 1; i < labels.length; i++) {
        const prev = byDecade[labels[i - 1]], cur = byDecade[labels[i]];
        if (cur - prev > jumpSize) { jumpSize = cur - prev; jumpFrom = labels[i - 1]; jumpTo = labels[i]; }
    }

    const total = events.length;
    const first = labels[0], last = labels[labels.length - 1];
    let text = `${total.toLocaleString()} records span the ${first}s to the ${last}s.`;
    if (jumpFrom !== null) {
        text += ` The largest change between consecutive decades is the ${jumpFrom}s to the ${jumpTo}s `
              + `(${byDecade[jumpFrom].toLocaleString()} to ${byDecade[jumpTo].toLocaleString()} records). `
              + `NHIRA does not treat that as evidence of a change in incidence — shifts of this size in a `
              + `historical archive usually reflect source availability and collection practice.`;
    }
...
function renderCharts(matches) {
    ...
    const fatByCountry = topEntries(sumBy(matches, e => e.country, e => e.fatalityCount), 10);
    drawChart("chartFatalitiesByCountry", {
        type: "bar",
        data: { labels: fatByCountry.map(([c]) => c), datasets: [{ data: fatByCountry.map(([, v]) => v), backgroundColor: "#D97A17" }] },
        options: { ...CHART_BASE_OPTIONS, indexAxis: "y" }
    });
    ...
    const incByCategory = countBy(matches, e => e.resolvedType);
    const catLabelsRanked = Object.keys(incByCategory).sort((a, b) => incByCategory[b] - incByCategory[a]);
    drawChart("chartIncidentsByCategory", {
        type: "bar",
        data: {
            labels: catLabelsRanked.map(k => (INCIDENT_TYPES[k] && INCIDENT_TYPES[k].label) || k),
            datasets: [{
                data: catLabelsRanked.map(k => incByCategory[k]),
                backgroundColor: catLabelsRanked.map(k => (INCIDENT_TYPES[k] && INCIDENT_TYPES[k].color) || "#888")
            }]
        },
        options: { ...CHART_BASE_OPTIONS, indexAxis: "y" }
    });

    const fatByCategory = sumBy(matches, e => e.resolvedType, e => e.fatalityCount);
    const fatCatLabelsRanked = Object.keys(fatByCategory).sort((a, b) => fatByCategory[b] - fatByCategory[a]);
    drawChart("chartFatalitiesByCategory", {
        type: "bar",
        data: {
            labels: fatCatLabelsRanked.map(k => (INCIDENT_TYPES[k] && INCIDENT_TYPES[k].label) || k),
            datasets: [{
                data: fatCatLabelsRanked.map(k => fatByCategory[k]),
                backgroundColor: fatCatLabelsRanked.map(k => (INCIDENT_TYPES[k] && INCIDENT_TYPES[k].color) || "#888")
            }]
        },
        options: { ...CHART_BASE_OPTIONS, indexAxis: "y" }
    });
...
function renderResearch(matches) {
    ...
    const avgFatalities = matches.length ? totalFatalities / matches.length : 0;
    const medianFatalities = median(matches.map(e => e.fatalityCount));
    ...
    rsSummary.innerHTML = `
        ...
        <div class="stat-card"><b>${avgFatalities.toFixed(1)}</b><span>Avg. Fatalities / Incident</span></div>
        <div class="stat-card"><b>${medianFatalities.toFixed(1)}</b><span>Median Fatalities / Incident</span></div>
    `;
...
const RESEARCH_FINDINGS_REGISTRY = {
    ...
    forecastClockCalibration: {
        title: "Forecast Clock (7/10/15-day EWMA) — Calibration",
        verdict: { "United States": "NOT SUPPORTED", Canada: "NOT SUPPORTED (never reached calibration)" },
        scope: "Whether NHIRA's short-horizon (7/10/15-day) elevated-activity probability is calibrated and ready to display live. Builds directly on production Model C — not gated behind Model D, since D2-D6 are rejected/non-production and no actual D-layer dependency exists.",
        evidence: [
            "US: EWMA beat the calendar-position baseline at all three horizons (Brier improvement 0.074/0.113/0.185 for 7/10/15 days) — but full-history calibration testing across 4 walk-forward windows returned NOT SUPPORTED at 7/10-day and INCONCLUSIVE at 15-day; only the most recent of 4 windows passed.",
            "US: a follow-up experiment restricted to the same dense, modern era the passing window came from — reusing the pre-existing outer-train/outer-test boundary rather than picking a new cutoff after seeing the result — again returned NOT SUPPORTED/INCONCLUSIVE. The pattern points to accumulated training volume, not calendar era, as the actual constraint.",
            "US: a simple geographic-concentration signal beat baseline alone, but added nothing once combined with EWMA (tested directly, not assumed) — confirming the map's heat-overlay layer stays correctly disabled.",
            "Canada: EWMA failed to beat baseline at all three horizons — the pipeline closed there before ever reaching a calibration test.",
            "US: six further model families tested against the same 4 walk-forward windows — a probability floor on EWMA, single-day (non-windowed) prediction, a three-component median ensemble, logistic regression on lagged binary indicators, gradient boosted trees, and a self-exciting Hawkes process. Every one beat baseline; none passed calibration in any window.",
            "US: a second, stronger ensemble — median of floor-EWMA, GBT, and Hawkes, the three best individual performers — produced the best calibration chi-square of any model tested (as low as 61 vs. 100-3465 for single models), and specifically smoothed out Hawkes' instability. Still, this is a smaller failure, not a near-miss: the corresponding p-values (1e-13 to 1e-100) are nowhere close to the p>=0.05 threshold, and the result holds at both 5 and 10 calibration bins. Reported precisely to avoid overstating how close it came.",
            "The mechanistic failure is consistent across nearly all of them: predicted probability runs below the true observed rate, most severely right after quiet stretches — e.g. one model predicted 0.04% risk following an inactive period where the real 15-day rate was 6.7%. A probability floor and a Hawkes process (which explicitly models self-excitation) each reduced this gap but did not close it.",
            "The Hawkes process produced the lowest Brier scores of any model tested, but its fitted branching-ratio parameter sat at the edge of the stability bound in most windows — a sign of likely overfitting given ~200-250 events per training window, not a stable, trustworthy estimate. Treated as a negative result, not a near-miss."
        ],
        methodologicalGuarantees: [
            "Every walk-forward prediction confirmed leakage-free by direct test: distorting future data left earlier predictions byte-identical.",
            "Decay parameter, spatial threshold, and blend weight all selected using only training-window data, never the held-out evaluation blocks.",
            "No threshold introduced after any result; the recent-era boundary reused an existing pre-registered split rather than being chosen after seeing which era passed."
        ],
        implication: "No probability is shown live for either country's 7/10/15-day forecast — the Forecast Summary card states this plainly rather than displaying a number the data doesn't support. Seven structurally different model families now converge on the same conclusion, which strengthens confidence in the finding itself, not just the original one. Revisiting this is tied to the same lever as the annual calibration finding above: more accumulated data, not a different model.",
...
// =====================================================================
// US-OBS-01 — EXTERNAL COVERAGE IDENTIFICATION
// An identifiability finding, not a model result. Records that a whole
// class of architectures (latent-incidence / observation-layer models)
// cannot be built for the US from the current source structure —
// regardless of how well any of them might fit.
// =====================================================================

const US_OBS_01_FINDING = Object.freeze({
    title: "US External Coverage Identification",
    verdict: "NOT SUPPORTED",
    recordedDate: "2026-08-22",
    kind: "identifiability",

    scope: "Whether NHIRA's US record can be compared against the FBI Active Shooter series to estimate NHIRA's coverage of the underlying incident population. NOT a verdict on whether US coverage is complete or incomplete — only that FBI-based comparison cannot identify it from the current source structure.",

    evidence: Object.freeze([
        "527 of 601 post-2000 US records (87.7%) cite the FBI as a source.",
        "264 records carry the FBI as their ONLY source.",
        "Several periods are effectively FBI-complete: 2000-2009 and 2020-2022 are at or near 100% FBI-cited.",
        "An NHIRA/FBI capture ratio would therefore measure source-transcription completeness, not NHIRA's coverage of the underlying incident population — the denominator is substantially the same source NHIRA ingests.",
        "The 2000s-to-2010s record jump (115 to 359 records) aligns with FBI series ingestion, identifying it as a source-availability boundary rather than evidence of an incidence increase.",
        "2026 is a distinct sourcing regime: 24 US records, none citing the FBI, consistent with the FBI series ending earlier than NHIRA's most recent data."
    ]),

    implication: "Any architecture requiring separation of true incidence from unobserved US reporting — latent-rate observation models, and regime-switching, hierarchical, or spatiotemporal variants that depend on that separation — is NOT CURRENTLY IDENTIFIABLE for the US. Such a model would produce a latent-rate posterior determined largely by the prior placed on the reporting layer rather than by the data. This is a structural limit, not a modelling difficulty to be solved with a better fit.",

    whyNowcastingDoesNotTransfer: "Public-health nowcasting works because reporting delay is observable — a late-reported case still arrives, so the delay distribution is empirically estimable. NHIRA's unrecorded incidents never arrive. There is no delay distribution, only permanent absence.",

    canadaScope: Object.freeze({
        status: "OPEN — separate assessment permitted",
        basis: "Statistics Canada's 2010-2024 police-reported mass-casualty series is UCR-based and genuinely independent of NHIRA's sourcing.",
        requirement: "Requires record-level matching against a defensible NHIRA subset, not annual-total comparison — margins alone are uninterpretable when the two definitions differ.",
        hardLimit: "A Canadian coverage estimate CANNOT identify a US latent-incidence/reporting decomposition. A successful Canadian comparison must never become the justification for a US observation model."
    }),

    doesNotEstablish: Object.freeze([
        "does not establish that US incident coverage is complete",
        "does not establish that US incident coverage is incomplete",
        "does not invalidate the FBI series as a data source — it remains a strong primary source",
        "does not bear on COUNT-01, which uses only identifiable observed-count quantities"
    ]),

    remainingIdentifiablePaths: Object.freeze([
        "COUNT-01 — identifiable count estimand on observed records",
        "Regression-to-the-mean / base-rate diagnostic for the quiet-period underprediction pattern",
        "Hierarchical partial pooling, if its assumptions can be evaluated without a reporting layer",
        "External predictors, subject to the D2 precedent",
        "Canadian coverage assessment as a bounded finding in its own right"
    ])
});
...
// =====================================================================
// DUR-01 — Duration Dependence vs Hawkes: Incremental Information Test
// EXECUTED 2026-08-27. Result: PRE-DECLARED NULL NOT REJECTED — duration
// dependence provides NO incremental information beyond a properly-fit
// Hawkes model. Recorded with the near-miss that a first, unstable Hawkes
// fit produced the OPPOSITE (false-positive) result before the frozen
// Hawkes-fairness check caught it.
// =====================================================================
const DUR_01_RESULT = Object.freeze({
    id: "DUR-01",
    title: "Duration Dependence vs Hawkes — Incremental Information",
    executed: "2026-08-27",
    status: "EXECUTED — NULL NOT REJECTED",
    prerequisite: "IA-01 (constant hazard rejected) + COUNT-01 (baseline closed)",
    computedAgainstArtifact: "1,031-record history.json (raw SHA-256 e54be167bad4cbb2c5db14000e4e738463719c2c137e9bfad4bd1ad0ec38d64b); COUNT-01 eligible US 2000-2025 set, 578 distinct event-days.",

    frozenQuestion: "Does elapsed time since the previous incident (a renewal/duration hazard) provide INCREMENTAL out-of-sample information beyond the tested Hawkes formulation and the frozen COUNT-01 baseline, on the same walk-forward/RPSS/calibration machinery?",
    preDeclaredNull: "Duration provides no incremental information beyond Hawkes: its skill is indistinguishable from the Hawkes model's, and neither clears the p>=0.05 calibration bar.",

    // ---- LAYER 1: the result as specified ----
    result: Object.freeze({
        verdict: "NULL NOT REJECTED. Once the Hawkes model is fit with a stable EM estimator, Hawkes DOMINATES the duration model at every horizon. Duration beats the baseline but does NOT beat Hawkes — it is the weaker representation of the same clustering IA-01 found.",
        rpsByHorizon: Object.freeze([
            "7-day:  baseline 0.3502 | Hawkes 0.2437 | Weibull 0.2987",
            "10-day: baseline 0.3632 | Hawkes 0.2114 | Weibull 0.2756",
            "15-day: baseline 0.3035 | Hawkes 0.1284 | Weibull 0.2066"
        ]),
        incrementalTest: Object.freeze([
            "Weibull - Hawkes dRPS (positive = Hawkes better): +0.0550 [CI +0.018,+0.099] (7d); +0.0642 [+0.031,+0.102] (10d); +0.0783 [+0.047,+0.112] (15d)",
            "All three CIs exclude zero IN FAVOUR OF HAWKES. The frozen decision rule therefore does NOT reject the null that duration adds nothing beyond Hawkes."
        ]),
        hawkesVsBaseline: "Hawkes - Baseline dRPS = -0.107/-0.152/-0.175 (7/10/15d) — Hawkes clears the shared baseline decisively, so the pairwise duration-vs-Hawkes comparison IS interpretable under the shared-anchor rule.",
        hawkesStability: "EM Hawkes branching ratio 0.84-0.91 across the four folds, converging to identical values from any initialization (std ~3e-16 on the full series). This is the fair, stable fit the spec required."
    }),

    // ---- LAYER 2: mandatory caveats ----
    caveats: Object.freeze({
        noCalibration: "No model, including the winning Hawkes, clears the p>=0.05 calibration bar — every reliability chi-square has p~0. This is RELATIVE skill (Hawkes orders better than duration orders better than baseline), NOT a calibrated or deployable forecast. The public PENDING VALIDATION status is unchanged.",
        effectiveUnits: "~10 effective units per horizon (paired circular block bootstrap, same ACF-driven ceiling as COUNT-01). CIs exclude zero but rest on modest independent information.",
        relativeNotAbsolute: "The finding is comparative. It narrows IA-01's mechanism question — the clustering is better captured by self-excitation than by renewal/duration structure — it does not deliver a working national-timing forecast."
    }),

    // ---- LAYER 3: the near-miss — recorded so the trap is documented ----
    executionIntegrityNote: Object.freeze({
        whatHappened: "The FIRST execution attempt used a direct-NLL Hawkes optimizer that was unstable: across 39 refits its branching ratio ranged 0.000-0.955 depending on initialization (median 0.000, i.e. it frequently collapsed Hawkes to near-Poisson). Against that broken Hawkes, the duration model appeared to WIN at every horizon (Weibull-Hawkes CI excluded zero in favour of Weibull) — a FALSE POSITIVE that would have rejected the null.",
        whatCaughtIt: "The frozen spec required a Hawkes-fairness/stability check BEFORE recording. The check failed (branching ratio init-dependent), so the result was NOT recorded. The optimizer was replaced with a convergent EM estimator; the re-run reversed the sign of the incremental test.",
        lesson: "A false positive was prevented by the pre-registered requirement to verify the comparison baseline before recording. Had the first result been filed, the registry would falsely claim duration beats Hawkes. Recorded here as a standing caution: model-comparison findings require the COMPARATOR to be validated, not just the challenger.",
        reconstructionDisclosure: "The Hawkes formulation was reconstructed from IA-01's description, not loaded from the original prior-work implementation. The EM fit's stability (init-independent convergence) is the evidence that the reconstructed comparator is now trustworthy."
    }),

    doesNotAuthorize: "No map display, no localization, no spatial/spatiotemporal extension (IA-01: clustering is national, not spatial). No COUNT-02. A null result authorizes nothing new; it closes IA-01's duration-vs-Hawkes sub-question.",
    closes: "IA-01 sharpenedQuestion (duration-vs-Hawkes). The mechanism behind the short-gap clustering is better represented by self-excitation than by renewal/duration dependence — though neither yields a calibrated forecast."
});
...
const IA_01_FINDING = Object.freeze({
    ...
    kind: "descriptive",
    recordedDate: "2026-08-22",
    replication: "RECOMPUTED 2026-08-24 on the 1,031-record artifact (610 eligible records, up from 577) using the identical frozen method. VERDICT UNCHANGED: chi2 181.3 (records-as-events) / 174.6 (collapsed) vs 186.3 / 180.6 prior; var/mean 49.8 vs ~51; 2-3 day gaps 98 observed vs 32.6 expected (was 94 vs 30.1); 45+ gaps 39 vs 36.4 expected, still near expectation. Notably, adding the 33 FBI 2025 incidents closed the eight-month terminal data gap that sat inside the original series, and the long-gap bin did NOT move — retroactive confirmation that the data boundary was not producing the clustering. The finding is replicated, not merely carried forward.",
    computedAgainstArtifact: "998-record history.json (825,598 bytes; raw SHA-256 dd5357a3e7dec18d785cda60104ef4339a43484ce7a82c50d10cffef3dfbffb7; canonical SHA-256 c6c7b1842a6350426b0e0cbf2fce7e9062eb7214e1a67cd6ba9ffcf55605e301). SUPERSEDED for current site data: on 2026-08-24 the 33 uningested FBI 2025 Active Shooter Report incidents were added, producing a 1,031-record artifact (860,112 bytes; raw SHA-256 e54be167bad4cbb2c5db14000e4e738463719c2c137e9bfad4bd1ad0ec38d64b). This finding remains valid AS EXECUTED against the artifact named above and was NOT recomputed. Any figure here describes the 998-record file, not the live dataset.",
...
// =====================================================================
// SEV-01 — SEVERITY TAIL ESTIMABILITY (DESCRIPTIVE)
// A precision finding, not a model result. The severity estimand was
// examined because it sidesteps the timing problem that closed the
// frequency programme. The distributional prerequisite is MET; the
// estimation precision is NOT. Recorded so the direction is closed on
// evidence rather than left as an untried possibility.
// =====================================================================

const SEV_01_FINDING = Object.freeze({
    id: "SEV-01",
    title: "Severity Tail Estimability",
    kind: "descriptive",
    verdict: "PREREQUISITE MET / NOT ESTIMABLE AT USEFUL PRECISION",
    recordedDate: "2026-08-24",

    motivation: "Literature on rare catastrophic events forecasts the SEVERITY distribution rather than event timing (Duwe/Sanders et al., J. Quantitative Criminology 2021, 156 US mass public shootings 1976-2018). Severity is a genuinely different estimand: it is conditional on occurrence and therefore does not require predicting when anything happens — the constraint that closed the seven-family frequency programme. NHIRA already holds fatalities and injuries on every record, so no new data was required.",

    replication: "RECOMPUTED 2026-08-24 on the 1,031-record artifact using the identical frozen method. VERDICT UNCHANGED: total-shot alpha 2.70 (was 2.69), x_min still 18, n_tail 33 (was 32), bootstrap CI [2.23, 3.49] (was [2.21, 3.54]); killed still favors lognormal. The estimability verdict is now better evidenced than when first recorded: 33 additional incidents — a 6% increase in records — added exactly ONE observation above x_min, and the largest ten total-shot values are identical, with Las Vegas (473) still roughly five times the next largest and still moving alpha to 3.01 when dropped. Tail sparsity is demonstrated here rather than asserted.",
    computedAgainstArtifact: "998-record history.json (825,598 bytes; raw SHA-256 dd5357a3e7dec18d785cda60104ef4339a43484ce7a82c50d10cffef3dfbffb7; canonical SHA-256 c6c7b1842a6350426b0e0cbf2fce7e9062eb7214e1a67cd6ba9ffcf55605e301). SUPERSEDED for current site data: on 2026-08-24 the 33 uningested FBI 2025 Active Shooter Report incidents were added, producing a 1,031-record artifact (860,112 bytes; raw SHA-256 e54be167bad4cbb2c5db14000e4e738463719c2c137e9bfad4bd1ad0ec38d64b). This finding remains valid AS EXECUTED against the artifact named above and was NOT recomputed. Any figure here describes the 998-record file, not the live dataset.",
    method: "Clauset-Shalizi-Newman: KS-minimising x_min, MLE tail exponent, likelihood-ratio comparison against exponential and lognormal fitted on the same tail. Primary series: COUNT-01 eligible set (US, 2000-2025). Descriptive only; no records changed, no specification frozen.",

    distributionalResult: Object.freeze({
        totalShot: "US 2000-2025, n=539 non-zero: POWER-LAW FAVORED. x_min=18, alpha=2.69, n_tail=32, KS=0.069. Beats exponential by 12.8 and lognormal by 14.3 log-likelihood.",
        killed: "US 2000-2025, n=412 non-zero: LOGNORMAL FAVORED, decisively (-68.3 vs power-law). The two severity measures do NOT behave alike — any future specification must select one measure and justify it rather than treating 'severity' as a single quantity.",
        fullDataset: "Power-law favored on both measures, but that series mixes countries and eras. Context only, not evidence for the US estimand.",
        conclusion: "The heavy-tail prerequisite for extreme-value machinery is MET for total-shot and NOT met for killed."
    }),

    estimabilityResult: Object.freeze({
        headline: "alpha = 2.69, 95% bootstrap CI [2.21, 3.54], width 1.33 (2000 resamples of the 32-point tail, seed 42).",
        whyItMatters: "In a power-law tail the exponent governs every downstream exceedance probability, and those probabilities are highly sensitive to it. An alpha uncertain across 2.21-3.54 yields severity forecasts whose intervals span orders of magnitude at the extremes — the same phenomenon reported in the source literature, whose headline carried a 90% interval of [8, 72] around a 35% point estimate.",
        leaveOneOut: Object.freeze([
            "full series: x_min=18, alpha=2.69, n_tail=32",
            "drop largest (Las Vegas 2017, total shot 473): alpha=3.02 (+0.34)",
            "drop largest 2 (473, 102): alpha=3.18 (+0.49)",
            "drop largest 3: alpha=3.31 (+0.62)"
        ]),
        jackknife: "Across all 32 tail points, alpha ranges 2.64-2.98; the single most influential observation is the 473, which alone moves alpha to 2.98.",
        tailSparsity: "Above x_min=18 there are 32 incidents; 4 at >=50 total shot and 2 at >=100. The largest ten total-shot values are 36, 38, 45, 45, 46, 49, 55, 70, 102, 473 — the maximum sits roughly five times above the next largest.",
        conclusion: "Three records out of 539 drive a ~23% drift in the exponent, and the bootstrap interval is wide independently of that. The tail is too sparse to estimate the parameter the method depends on."
    }),

    doesNotEstablish: Object.freeze([
        "does not establish that severity is unmodellable in principle — only that THIS record cannot estimate the tail exponent at useful precision",
        "does not contradict the published severity literature; it reproduces that literature's own stated sensitivity to tail-location choice on a smaller sample",
        "does not bear on the frequency findings, which fail for a different reason (timing predictability, not tail sparsity)",
        "no severity specification is authorized, frozen, or scheduled by this entry"
    ]),

    programmePattern: "SEV-01 completes a three-way pattern the registry can now state plainly: the frequency estimand fails on timing predictability (seven families, none calibrated); the severity estimand fails on tail sparsity (this entry); the US observation layer fails on identifiability (US-OBS-01). Three structurally different routes, three different walls — all traceable to 577 records being a small number for questions of this difficulty. This is a finding about the evidence base, not about any particular model.",

    reopenCondition: "If the eligible record grows materially in the upper tail — through era extension, additional independent sourcing, or a defensible multi-country pooling with its own frozen specification — the estimability question is worth re-running. The distributional prerequisite would not need re-establishing; only the precision."
});
...
function computeModelStatusPanel() {
    ...
    const forecastClockFinding = RESEARCH_FINDINGS_REGISTRY.forecastClockCalibration;
    rows.push({
        component: forecastClockFinding.title,
        level: "red",
        status: `US: ${forecastClockFinding.verdict["United States"]} · Canada: ${forecastClockFinding.verdict.Canada}`,
        detail: forecastClockFinding.implication
    });
    ...
    rows.push({
        component: "SHF-1A — Space-Time Self-Excitation Diagnostic",
        level: "blocked",
        status: SHF_1A_FINDING.status,
        detail: `Not a verdict — the frozen protocol was never validly executed. ${SHF_1A_FINDING.preconditionsUnmet.length} preconditions unmet; blocked by ${SHF_1A_FINDING.blockedBy}. The exploratory run (B=${SHF_1A_FINDING.exploratoryArtifact.permutations}, not the frozen B=10000) reached only ${SHF_1A_FINDING.exploratoryArtifact.evaluableCells["United States"]}/25 evaluable cells for the US and ${SHF_1A_FINDING.exploratoryArtifact.evaluableCells.Canada}/25 for Canada, against a required ${SHF_1A_FINDING.exploratoryArtifact.evaluableCells.required}.`
    });

    rows.push({
        component: NHIRA_DQ_01.item,
        level: "orange",
        status: NHIRA_DQ_01.status,
        detail: `Spatial provenance and incident-identity backfill — ${NHIRA_DQ_01.deliverables.length} deliverables. Blocks ${NHIRA_DQ_01.blocks.join(" and ")}.`
    });

    rows.push({
        component: `US-OBS-01 — ${US_OBS_01_FINDING.title}`,
        level: "red",
        status: US_OBS_01_FINDING.verdict,
        detail: `${US_OBS_01_FINDING.evidence[0]} ${US_OBS_01_FINDING.evidence[1]} An NHIRA/FBI capture ratio would measure source transcription, not coverage of the underlying incident population — so latent-incidence/observation-layer architectures are not currently identifiable for the US. Canada remains separately assessable and cannot substitute.`
    });

    rows.push({
        component: `${COUNT_01_SPEC.id} — ${COUNT_01_SPEC.title}`,
        level: "yellow",
        status: COUNT_01_RESULT.status,
        detail: `Executed 2026-08-22. Frozen result RPSS +0.54/+0.51/+0.45 (7/10/15-day) — but the artifact decomposition shows 95-98% of that headline is a baseline-smoothing artifact and must never be quoted without it. Post-hoc sensitivity (the genuine content): RPSS +0.009/+0.015/+0.022, monotone in horizon, descriptively excluding zero, ~10 effective bootstrap units. There is no pass; nothing is deployed.`
    });

    rows.push({
        component: `${IA_01_FINDING.id} — ${IA_01_FINDING.title}`,
        level: "yellow",
        status: "Descriptive finding — constant hazard rejected",
        detail: `Short-gap clustering (2-3 day gaps ~3x expected, var/mean ~52) on the COUNT-01 eligible series, robust to same-day definition and to the near-duplicate sweep (2 plausible residual duplicates vs ~64-event excess). Not spatial — clustering operates nationally. Establishes non-constant hazard only; no mechanism, no COUNT-02 authorization.`
    });

    rows.push({
        component: `${SEV_01_FINDING.id} — ${SEV_01_FINDING.title}`,
        level: "yellow",
        status: SEV_01_FINDING.verdict,
        detail: `Severity is a distinct estimand that sidesteps timing, and the heavy-tail prerequisite IS met for total-shot (power-law favored, alpha=2.69) though NOT for fatalities (lognormal favored). But the tail holds only 32 incidents: dropping Las Vegas alone moves alpha to 3.02, and the bootstrap CI is [2.21, 3.54]. Too sparse to estimate the parameter the method depends on. Descriptive; nothing specified or scheduled.`
    });

    rows.push({
        component: `${NHIRA_DQ_01_L.item} (v${NHIRA_DQ_01_L.version})`,
        level: "blocked",
        status: `Methodology ${NHIRA_DQ_01_L.status} \u00b7 fingerprint ${NHIRA_DQ_01_L.fingerprint.status}`,
        detail: `Scoped to ${NHIRA_DQ_01_L.scope.countries.join(" + ")} (${NHIRA_DQ_01_L.scope.recordsPreCleanup.total} pre-cleanup records). ${NHIRA_DQ_01_L.perRecordLookup.distinctLookups} city-reference lookups; ${NHIRA_DQ_01_L.resolutionGate.usCanadaRouting.stage3Total} records (${NHIRA_DQ_01_L.resolutionGate.usCanadaRouting.pctOfSubset}%) route to source adjudication on resolution grounds alone. Identification thresholds frozen before testing; no candidate source identified yet.`
    });
...
function renderModelStatusPanel() {
    ...
    const icon = { green: "🟢", yellow: "🟡", orange: "🟠", red: "🔴", gray: "⚪", locked: "🔒", blocked: "🚧" };
...
function renderForecastClockEvidence() {
    const finding = RESEARCH_FINDINGS_REGISTRY.forecastClockCalibration;
    if (!finding) return "";
    return `
        <details class="forecast-clock-evidence">
            <summary>Full evidence — every model tested for the Forecast Clock, and why each was rejected</summary>
            <p class="meta">Recorded ${escapeHtml(finding.recordedDate)}. Scope: ${escapeHtml(finding.scope)}</p>
            <ul class="evidence-list">
                ${finding.evidence.map(e => `<li>${escapeHtml(e)}</li>`).join("")}
            </ul>
            <p class="chart-title">Methodological guarantees</p>
            <ul class="evidence-list">
                ${finding.methodologicalGuarantees.map(g => `<li>${escapeHtml(g)}</li>`).join("")}
            </ul>
        </details>
    `;
}
...
//
// IMPORTANT: this function reads `events`, which is only populated
// once history.json has actually loaded (fetch is async — this file
// executes synchronously well before that resolves). It must be
// called from inside the fetch's success handler above, never as a
// standalone top-level call here — a top-level call would run against
// the initial empty `events = []` and never re-run once real data
// arrives, silently leaving Live Statistics/charts blank.
// ---------------------------------------------------------------------
...
function populateDashboardShell() {
    ...
    const regionSelect = document.getElementById("dashFilterRegion");
    if (regionSelect) {
        const regions = [...new Set(events.map(e => getRegion(e.country)))].sort();
        regions.forEach(r => {
            const opt = document.createElement("option");
...
function matchesActiveDashFilters(event) {
    ...
    const regionEl = document.getElementById("dashFilterRegion");
    ...
    if (regionEl && regionEl.value && getRegion(event.country) !== regionEl.value) return false;
...
if (DASH_FILTER_ELEMENTS_PRESENT) {
    const dashFilterIds = ["dashFilterCountry", "dashFilterCategory", "dashFilterRegion", "dashFilterMinFatalities", "dashStartYear", "dashEndYear"];
...
// Note: populateDashboardShell() is NOT called here — it must run
// after history.json has actually loaded, from inside the fetch's
// success handler near the top of this file. Calling it here would
// run against the initial empty `events = []`.
...
// sync with which drawer is actually open, and keeps aria-current in
// sync with that same class so assistive tech reflects the same
// state a sighted user sees, not just the visual highlight.
