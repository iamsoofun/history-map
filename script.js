...
function renderBacktestTable(backtest, dispersionRatio, country) {
    ...
    return `
        ...
        <p class="meta">
            This is the naive two-way split: interval built from the earliest ~70% of backtested years, checked
            against the most recent ~30% it never saw. This is what exposed the overconfidence problem.
        </p>
        <dl class="forecast-fields">
            ${[0.8, 0.9].map(cov => {
        ...
        <dl class="forecast-fields">
            ${[0.8, 0.9].map(cov => {
                ...
                return `
                    ...
                    <b>${recal.wellCalibrated ? "reasonably calibrated" : "still off target"}</b>.</dd>
                `;
            }).join("")}
        </dl>
