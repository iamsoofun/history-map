...
function renderBacktestTable(backtest, dispersionRatio, country) {
    ...
    return `
        ...
        <p class="chart-title">Alternative construction — Negative Binomial quantile interval</p>
        <p class="meta">
            A statistically principled alternative: instead of empirical residual quantiles + a searched multiplier, fit
            a Negative Binomial distribution's dispersion parameter from TRAINING years only, then read interval bounds
            directly from its quantile function. Same walk-forward discipline, same held-out test years.
        </p>
        ${(() => {
            const nb80 = computeNBInterval(backtest.results, 0.8);
            const nb90 = computeNBInterval(backtest.results, 0.9);
            const recal80cmp = computeRecalibratedInterval(backtest.results, 0, 0.8);
            const recal90cmp = computeRecalibratedInterval(backtest.results, 0, 0.9);
            if (!nb80 || !nb90) return `<p class="dq-empty">Not enough backtested years for the NB comparison yet.</p>`;
            return `
                <table class="backtest-table">
                    <thead><tr><th>Target</th><th>Empirical + multiplier</th><th>Negative Binomial</th><th>Better on this data</th></tr></thead>
                    <tbody>
                        <tr>
                            <td>80%</td>
                            <td>${recal80cmp ? recal80cmp.testCoveragePct + "%" : "—"}</td>
                            <td>${nb80.testCoveragePct}% <span class="backtest-range">(r=${nb80.rParameter}, dispersion=${nb80.dispersionRatio})</span></td>
                            <td>${recal80cmp ? (Math.abs(recal80cmp.testCoveragePct - 80) <= Math.abs(nb80.testCoveragePct - 80) ? "Empirical" : "NB") : "—"}</td>
                        </tr>
                        <tr>
                            <td>90%</td>
                            <td>${recal90cmp ? recal90cmp.testCoveragePct + "%" : "—"}</td>
                            <td>${nb90.testCoveragePct}% <span class="backtest-range">(r=${nb90.rParameter}, dispersion=${nb90.dispersionRatio})</span></td>
                            <td>${recal90cmp ? (Math.abs(recal90cmp.testCoveragePct - 90) <= Math.abs(nb90.testCoveragePct - 90) ? "Empirical" : "NB") : "—"}</td>
                        </tr>
                    </tbody>
                </table>
                <p class="review-criteria-note">
                    Whichever column is closer to its target on THIS real data is the legitimate answer — this isn't
                    picked in advance. In synthetic stress-testing during development, the empirical+multiplier approach
                    outperformed this NB implementation even in scenarios built specifically to favor NB (stable, non-shifting
                    overdispersion) — worth treating that as a caution about this particular NB implementation's dispersion-fitting
                    method, not proof either way for your real data.
                </p>
            `;
        })()}
        ...
        <p class="meta">
            Real FBI-sourced violent-crime and homicide rates, but honestly PARTIAL: only 6 of 51 US states/DC are
            covered (Alaska, Alabama, Arkansas, Arizona, California, Colorado — the portion of a larger source file
            that was actually retrievable and verified). This tests whether the surrounding crime-rate environment
            (a signal genuinely distinct from NHIRA's own incident-count trend) adds anything, using only incidents in
            covered states — never guessed or extrapolated to uncovered ones.
        </p>
        ${(() => {
...
function renderForecast(country) {
    ...
    const recalInterval = cachedBacktest && !cachedBacktest.insufficientData
        ? computeRecalibratedInterval(cachedBacktest.results, result.modelEstimate, 0.9)
        : null;
