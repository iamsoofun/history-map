...
function computeModelStatusPanel() {
    ...
    rows.push({
        ...
        level: "gray",
        status: COUNT_01_SPEC.status,
        detail: `Estimation study — no pass/fail, no alpha, no deployment authorization. Era ${COUNT_01_SPEC.era.start} to ${COUNT_01_SPEC.era.end} (${COUNT_01_SPEC.era.eligibleRecords} US records), boundary set by FBI collection mandate rather than by distribution. Descriptive gate PASSED: >=2-incident blocks are ${COUNT_01_SPEC.gate.rows.map(r => r.pctTwoPlus + "%").join(" / ")} at ${COUNT_01_SPEC.gate.rows.map(r => r.horizon).join(" / ")}, so E[count] is distinct from P(>=1).`
    });
