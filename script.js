...
// Deliberately EMPTY: no country's interval calibration has been
// formally reviewed and locked yet. Every country shows "UNDER REVIEW"
// until a human adds an entry here after actually reviewing the real
// coverage numbers — never auto-populated by the code itself.
// =====================================================================
...
function computeC512FormalComparison(backtest) {
    ...
    if (!comparison80.empirical || !comparison80.nb || !comparison90.empirical || !comparison90.nb) {
        return { comparison80, comparison90, decision: { verdict: "insufficient", text: "Not enough backtested years to run the full formal comparison yet." } };
    }
    ...
    return { comparison80, comparison90, decision };
}
    ...
    return `
        ...
        ${(() => {
            ...
            function methodRows(m) {
                if (!m) return `<tr><td colspan="7">Not computable</td></tr>`;
                return `
                    <tr>
                        <td>${m.method}</td>
                        <td>${m.coveragePct}%</td>
            ...
            return `
                ...
                <table class="backtest-table ablation-table">
                    <thead><tr><th>Method</th><th>Coverage</th><th>Avg. width</th><th>Winkler score</th><th>Calibrated?</th><th>1st/2nd half coverage</th><th>Sample requirement</th></tr></thead>
        ...
        <div class="model-status-badge model-status-gray">
            <span class="model-status-label">RESEARCH / PARTIAL-COVERAGE FACTOR — NOT PRODUCTION</span>
            <p>Real FBI-sourced data, but only 6 of 51 US states/DC are covered (Alaska, Alabama, Arkansas, Arizona,
            California, Colorado). This stays a research-only factor — never folded into the live Risk Score or the
            production forecast — until the complete state/year dataset is sourced. No amount of good results on this
            partial sample changes that; coverage completeness is the gate, not statistical performance.</p>
        </div>
        <p class="meta">
        ...
        <p class="forecast-disclaimer">
            ...
            not a prediction that an incident will occur. ${(() => {
                ...
                if (c51.status === "not_run") {
                    return `The count/classification model has not yet been backtested in this session — run a backtest below to see how it performs against historical outcomes.`;
                }
