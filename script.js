...
const KNOWN_ISSUES_REGISTRY = {
    usThresholdInstability: {
        title: "US classification threshold instability",
        scope: "United States — Model C's elevated-year classification threshold",
        status: "Documented, unresolved",
        evidence: [
            "C.1/C.2 robustness audit: precision swings 65.8pp (21.7% → 87.5%) across a ±10 move from the chosen threshold of 45.",
            "D1 threshold-fairness test: Model C's own F1-optimal threshold, selected from training data, produces a degenerate operating point — 100% recall, 66.7% precision, 100% false-positive rate — flagging every non-elevated held-out year as elevated too."
        ],
        implication: "The US model does not currently have a stable, well-behaved classification threshold to tune toward — this is a property of the underlying model/data, not something fixable by adjusting D1 or any ensemble built on top of it. Surfaced independently by two different tests, which is why it's tracked here rather than treated as a one-off result.",
...
function computeModelStatusPanel() {
    ...
    const usIssue = KNOWN_ISSUES_REGISTRY.usThresholdInstability;
    rows.push({ component: usIssue.title, level: "orange", status: usIssue.status, detail: usIssue.implication });
